import { CODE_BLOCK_LANGUAGUE_SYNTAX_DEFAULT, CODE_BLOCK_LANGUAGUES } from '@/constants/code-languages';
...
export async function loadLanguage(languageName: string, lowlight: ReturnType<typeof createLowlight>) {
...
export function findLanguage(value: string | undefined) {
   ...
   const language = CODE_BLOCK_LANGUAGUES.find((language) => language.syntax === lowercase || language.alias?.split(', ').includes(lowercase));
   return language;
}
