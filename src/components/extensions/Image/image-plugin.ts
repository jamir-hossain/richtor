         ...
         return {
            update: async () => {
               const imageBlocks = findChildren(
                  view.state.doc,
                  (node) => node.type.name === name && (!node.attrs.naturalWidth || !node.attrs.naturalHeight),
               );

               if (imageBlocks.length === 0) return;

               const results = await Promise.all(imageBlocks.flatMap(({ node }) => getDimensionsImage(node.attrs.src)));
