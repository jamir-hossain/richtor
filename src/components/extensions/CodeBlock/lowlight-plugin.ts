...
import { loadLanguage } from '../../../lib/codeLanguageLoader';
      ...
      view(view) {
         class LowlightPluginView {
            ...
            async initDecorations() {
               ...
               const languages = [...codeBlocks.map((block) => block.node.attrs.language || CODE_BLOCK_LANGUAGUE_SYNTAX_DEFAULT), defaultLanguage];
            ...
            async checkUndecoratedBlocks() {
               ...
               const loadStates = await Promise.all(
                  codeBlocks.flatMap((block) => [loadLanguage(block.node.attrs.language || CODE_BLOCK_LANGUAGUE_SYNTAX_DEFAULT, lowlight)]),
               );
