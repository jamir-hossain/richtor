               ...
               if (mode === -1) {
                  chain()
                     .command(({ chain, tr }) => {
                        ...
                        if (tr.selection.empty) return chain().blur().focus(tr.selection.anchor).insertLink({ text: '\u200B' }).run();
                        // @ts-ignore
      ...
      return [
         ...
         new Plugin({
            ...
            appendTransaction: (transactions, oldState, newState) => {
               ...
               const skipTransaction = transactions.some((transaction) => transaction.getMeta('preventClearTempLink'));
...
function clearTempLinks(tr: any, doc: any, selection: any) {
   ...
   if (empty) {
      ...
      doc.nodesBetween(from, to, (node: any, pos: any) => {
         const linkMark = node.marks.find((mark: any) => mark.type.name === 'link' && !mark.attrs.href);
