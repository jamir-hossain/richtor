...
const TableBuilder = ({ onCreate }: TableBuilderProps) => {
   ...
   const isActiveCell = (rowIndex: number, colIndex: number) => rowIndex < gridSize.rows && colIndex < gridSize.cols;
         ...
         Array.from({ length: ROWS }, (_, rowIndex) => (
            <div key={`row-${rowIndex}`} className="rte-tb__row">
               {Array.from({ length: COLUMNS }, (_, colIndex) => (
                  <div
                     key={`col-${colIndex}`}
                     className={clsx('rte-tb__cell', isActiveCell(rowIndex, colIndex) && 'rte-tb__cell--active')}
