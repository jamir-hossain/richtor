...
import { getShortcutKey } from '../lib/shortcut';
...
const MenuButton = React.forwardRef<HTMLButtonElement, MenuButtonProps>(
   (
      ...
      const renderIcon = useMemo(() => (icon ? <Icon name={icon} className="rte-button-icon" /> : null), [icon]);
      ...
      const renderContent = tooltipContent ? (
         <Tooltip content={tooltipContent} options={{ collisionBoundary: contentElement.current?.parentElement }}>
            {renderButton}
         </Tooltip>
      ) : (
         renderButton
      ...
      if (type === 'dropdown') {
         return (
            <DropdownMenu modal={false}>
               ...
               <DropdownMenuContent className={dropdownClass} style={dropdownStyle} onCloseAutoFocus={(e) => e.preventDefault()}>
                  {children}
               </DropdownMenuContent>
            </DropdownMenu>
         );
      ...
      if (type === 'popover') {
         return (
            <Popover modal={false}>
               ...
               <PopoverContent className={dropdownClass} style={dropdownStyle} onCloseAutoFocus={(e) => e.preventDefault()}>
                  {children}
               </PopoverContent>
            </Popover>
         );
